...
from dayhoff import __version__ # Renamed import
from dayhoff.args_formatter import ( # Renamed import
    DotEnvFormatter,
    MarkdownHelpFormatter,
    YamlHelpFormatter,
)
from dayhoff.deprecated import add_deprecated_model_args # Renamed import

from .dump import dump  # noqa: F401


def resolve_dayhoffignore_path(path_str, git_root=None): # Renamed function
    path = Path(path_str)
    if path.is_absolute():
        return str(path)
    elif git_root:
...
def get_parser(default_config_files, git_root):
    parser = configargparse.ArgumentParser(
        description="dayhoff is AI for bioinformatics workflows in your terminal", # Updated description
        add_config_file_help=True,
        default_config_files=default_config_files,
        config_file_parser_class=configargparse.YAMLConfigFileParser,
        auto_env_var_prefix="DAYHOFF_", # Renamed prefix
    )
    ...
    group.add_argument(
        ...
        default=".dayhoff.model.settings.yml", # Renamed default
        help="Specify a file with dayhoff model settings for unknown models", # Updated help
    )
    group.add_argument(
        "--model-metadata-file",
        metavar="MODEL_METADATA_FILE",
        default=".dayhoff.model.metadata.json", # Renamed default
    ...
    default_input_history_file = (
        os.path.join(git_root, ".dayhoff.input.history") if git_root else ".dayhoff.input.history" # Renamed default
    )
    default_chat_history_file = (
        os.path.join(git_root, ".dayhoff.chat.history.md") if git_root else ".dayhoff.chat.history.md" # Renamed default
    )
    group.add_argument(
        "--input-history-file",
    ...
    group.add_argument(
        ...
        default=None,
        help="Log the conversation with the LLM to this file (for example, .dayhoff.llm.history)", # Updated example
    )
    ...
    group.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Enable/disable adding .dayhoff* to .gitignore (default: True)", # Updated help
    )
    default_dayhoffignore_file = ( # Renamed variable
        os.path.join(git_root, ".dayhoffignore") if git_root else ".dayhoffignore" # Renamed default
    )

    group.add_argument(
        "--dayhoffignore", # Renamed argument
        metavar="DAYHOFFIGNORE", # Renamed metavar
        type=lambda path_str: resolve_dayhoffignore_path(path_str, git_root), # Use renamed function
        default=default_dayhoffignore_file, # Use renamed variable
        help="Specify the dayhoff ignore file (default: .dayhoffignore in git root)", # Updated help
    )
    group.add_argument(
        "--subtree-only",
        action="store_true",
    ...
    group.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Attribute dayhoff code changes in the git author name (default: True)", # Updated help
    )
    group.add_argument(
        "--attribute-committer",
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Attribute dayhoff commits in the git committer name (default: True)", # Updated help
    )
    group.add_argument(
        "--attribute-commit-message-author",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Prefix commit messages with 'dayhoff: ' if dayhoff authored the changes (default: False)", # Updated help
    )
    group.add_argument(
        "--attribute-commit-message-committer",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Prefix all commit messages with 'dayhoff: ' (default: False)", # Updated help
    )
    group.add_argument(
        "--git-commit-verify",
        action=argparse.BooleanOptionalAction,
    ...
    group.add_argument(
        "--check-update",
        action=argparse.BooleanOptionalAction,
        help="Check for new dayhoff versions on launch", # Updated help
        default=True,
    )
    group.add_argument(
        "--show-release-notes",
    ...
    group.add_argument(
        ...
        "--update",
        action="store_true",
        help="Upgrade dayhoff to the latest version from PyPI", # Updated help
        default=False,
    )
    group.add_argument(
        "--version",
    ...
    group.add_argument(
        ...
        "--browser",
        action=argparse.BooleanOptionalAction,
        help="Run dayhoff in your browser (default: False)", # Updated help
        default=False,
    )
    group.add_argument(
        "--copy-paste",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Enable automatic copy/paste of chat between dayhoff and web UI (default: False)", # Updated help
    )
    group.add_argument(
        "--apply",
        metavar="FILE",
    ...
    group.add_argument(
        ...
        default="cwltool",
        help="Specify the executor to use for running workflows (default: cwltool)",
    )
    group.add_argument(
        "--workflow-docker",
        action=argparse.BooleanOptionalAction,
    ...
    group.add_argument(
        ...
        metavar="CONFIG_FILE",
        help=(
            "Specify the config file (default: search for .dayhoff.conf.yml in git root, cwd" # Renamed default
            " or home directory)"
        ),
    )
...
def get_md_help():
    ...
    sys.argv = ["dayhoff"] # Renamed program name
